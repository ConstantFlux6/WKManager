...
function renderTable(players) {
  ...
  document.getElementById("saveShift").addEventListener("click", async () => {
    ...
    rows.forEach(row => {
      ...
      shift1Assignments.push({ id, name, assignedTo, captain, backup, joiner });
    });

    const wkDate = getCurrentWK();
    const docRef = doc(db, "shift1", wkDate);
    try {
      await updateDoc(docRef, { assignments: shift1Assignments });
